...
namespace Eloom\AjaxLayeredNav\Helper;

class Data extends \Magento\Framework\App\Helper\AbstractHelper {
	protected $filterManager;

	const ENABLE = 'eloom_ajaxlayerednav/general/enable';
	const ENABLE_PRICE_SLIDER = 'eloom_ajaxlayerednav/general/enable_price_slider';

	const ENABLE_FILTER_BY_RATING = 'eloom_ajaxlayerednav/rating_builder/enable_filter';
	const ENABLE_SORT_BY_RATING = 'eloom_ajaxlayerednav/rating_builder/enable_sort';
	const RATING_CODE = 'rating';
	const RATING_CODE_PATH = 'eloom_ajaxlayerednav/rating_builder/rating_code';
	const RATING_FILTER_LABEL = 'eloom_ajaxlayerednav/rating_builder/filter_label';
	const RATING_SORT_LABEL = 'eloom_ajaxlayerednav/rating_builder/sort_label';
	const RATING_FILTER_TYPE_PATH = 'eloom_ajaxlayerednav/rating_builder/filter_type';
	const AVG_RATING_PERCENT = 'avg_percent';

	const ENABLE_FILTER_BY_STOCK_STATUS = 'eloom_ajaxlayerednav/stock_builder/enable_filter';
	const STOCK_STATUS_CODE = 'is_in_stock';
	const STOCK_STATUS_CODE_PATH = 'eloom_ajaxlayerednav/stock_builder/in_stock_code';
	const STOCK_FILTER_LABEL = 'eloom_ajaxlayerednav/stock_builder/filter_label';
	...
	public function getFilterHtml($filter, $customStyle) {
		...
		return $this->getLayout()->createBlock($block)
			->setTemplate('Eloom_AjaxLayeredNav::layer/custom-style/' . $customStyle . '.phtml')
	...
	public function enableMultiSelect() {
		if (null === $this->_enableMultiSelect) {
			$this->_enableMultiSelect = ((bool)$this->getConfig('eloom_ajaxlayerednav/general/enable_multiselect')) && ((bool)$this->enableAjaxLayeredNavigation());
		}
	...
	public function enableCategoryMultiSelect() {
		if ($this->_enableCategoryMultiSelect === null) {
			$this->_enableCategoryMultiSelect = $this->scopeConfig->getValue('eloom_ajaxlayerednav/general/category_multiselect', 'store')
	...
	public function getFilterByRatingHtml($filter) {
		...
		$block = $this->block;
		return $this->getLayout()->createBlock($block)
			->setTemplate('Eloom_AjaxLayeredNav::layer/rating/' . $customStyle . '.phtml')
			->setOptionsFilter($filter)
			->setData('custom_style', $customStyle)
			->toHtml();
	}
	...
	public function getRatingFilter($layer) {
		if ($this->ratingFilter === null) {
			$this->ratingFilter = $this->objectManager->create(
				\Eloom\AjaxLayeredNav\Model\Layer\Filter\Rating::class,
				['layer' => $layer]
			);
		}
	...
	public function getStockFilter($layer) {
		if ($this->stockFilter === null) {
			$this->stockFilter = $this->objectManager->create(
				\Eloom\AjaxLayeredNav\Model\Layer\Filter\StockStatus::class,
				['layer' => $layer]
			);
		}
